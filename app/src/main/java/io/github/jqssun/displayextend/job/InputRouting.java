...
public class InputRouting {
  ...
  public static void bindInputToDisplay(
      DisplayInfo displayInfo,
      InputDevice inputDevice,
      IInputManager inputManager,
      Map<String, String> inputDeviceDescriptorToPortMap) {
    ...
    try {
      ...
      inputManager.addUniqueIdAssociationByDescriptor(
          inputDevice.getDescriptor(), String.valueOf(displayInfo.uniqueId));
      State.log(
          "input routing updated: " + inputDevice.getName() + ", " + inputDevice.getDescriptor());
    } catch (Throwable e) {
      ...
      if (inputPort == null) {
        ...
        try {
          ...
          try {
            ...
            State.log(
                "input port fallback also failed: "
                    + inputDevice.getName()
                    + ", "
                    + e3.getMessage());
            if (ShizukuUtils.hasPermission() && State.currentActivity.get() != null) {
              Toast.makeText(
                      ...
                      State.currentActivity.get().getString(R.string.input_routing_limitation),
                      Toast.LENGTH_SHORT)
                  .show();
            }
          }
  ...
  public static void bindAllExternalInputToDisplay(int displayId) {
    ...
    Map<String, String> inputDeviceDescriptorToPortMap =
        InputRouting.getInputDeviceDescriptorToPortMap();
    for (int deviceId : inputManager.getInputDeviceIds()) {
      InputDevice inputDevice = inputManager.getInputDevice(deviceId);
      InputRouting.bindInputToDisplay(
          displayInfo, inputDevice, inputManager, inputDeviceDescriptorToPortMap);
    }
  }
}
