...
public class ChangeDPI implements Job {
  ...
  public void start() throws YieldException {
    ...
    if (!requestedConfirmation) {
      ...
      dialog =
          new MaterialAlertDialogBuilder(State.currentActivity.get())
              .setTitle(State.currentActivity.get().getString(R.string.edit_dpi))
              .setMessage(
                  State.currentActivity.get().getString(R.string.confirm_dpi_format, oldDpi, dpi))
              .setPositiveButton(
                  State.currentActivity.get().getString(R.string.ok),
                  (dialog, which) -> {
                    ChangeDPI.this.confirmed = true;
                    State.resumeJob();
                  })
              .setNegativeButton(
                  State.currentActivity.get().getString(R.string.cancel),
                  (dialog, which) -> {
                    State.resumeJob();
                  })
              .show();
