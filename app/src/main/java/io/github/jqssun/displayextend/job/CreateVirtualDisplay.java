...
public class CreateVirtualDisplay {
  ...
  private static VirtualDisplay createByMediaProjection(
      VirtualDisplayArgs virtualDisplayArgs, Surface surface) {
  ...
  private static @NonNull VirtualDisplay createByShizuku(
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
      displayId =
          displayManager.createVirtualDisplay(config, callback, projection, "com.android.shell");
    } else {
      displayId =
    ...
    VirtualDisplay virtualDisplay =
        DisplayManagerGlobal.getInstance().createVirtualDisplayWrapper(config, callback, displayId);
