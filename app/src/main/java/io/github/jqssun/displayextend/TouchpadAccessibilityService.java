...
public class TouchpadAccessibilityService extends AccessibilityService {
  ...
  private List<AccessibilityNodeInfo> _findActionableNodes(
      AccessibilityNodeInfo root, List<AccessibilityNodeInfo> results) {
  ...
  private AccessibilityWindowInfo _findTopActionableWindow(int displayId) {
    ...
    android.util.Log.d(
        ...
        "window list: "
            + (targetDisplayWindows != null ? targetDisplayWindows.size() : 0)
            + " windows");
    ...
    for (AccessibilityWindowInfo window : targetDisplayWindows) {
      ...
      try {
        List<AccessibilityNodeInfo> actionableNodes =
            _findActionableNodes(rootNode, new ArrayList<>());
  ...
  public boolean setFocus(int displayId) {
    ...
    android.util.Log.d(
        "AccessibilityService",
        "found top actionable window, type: "
            + topWindow.getType()
            + ", layer: "
            + topWindow.getLayer());
    ...
    android.util.Log.d(
        "AccessibilityService", "root node: " + (rootNode != null ? "found" : "null"));
    if (rootNode == null) {
      return false;
    }
    ...
    try {
      List<AccessibilityNodeInfo> actionableNodes =
          _findActionableNodes(rootNode, new ArrayList<>());
      android.util.Log.d(
          "AccessibilityService", "found " + actionableNodes.size() + " actionable nodes");
      ...
      for (AccessibilityNodeInfo node : actionableNodes) {
        try {
          ...
          android.util.Log.d(
              "AccessibilityService", "set focus: " + (focusResult ? "success" : "failed"));
          if (focusResult) {
            focusSuccess = true;
            break;
          }
  ...
  public void performBackGesture(int displayId) {
    ...
    android.util.Log.d(
        "AccessibilityService", "back action: " + (backResult ? "success" : "failed"));
  }
}
