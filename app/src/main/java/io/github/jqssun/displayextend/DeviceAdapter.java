...
public class DeviceAdapter extends RecyclerView.Adapter<DeviceAdapter.DeviceViewHolder> {
  ...
  @DrawableRes private final int deviceIconRes;

  public DeviceAdapter(
      List<InputDevice> devices, Consumer<InputDevice> onClick, @DrawableRes int deviceIconRes) {
  ...
  public DeviceViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    View view =
        LayoutInflater.from(parent.getContext()).inflate(R.layout.item_input_device, parent, false);
  ...
  public void onBindViewHolder(@NonNull DeviceViewHolder holder, int position) {
    ...
    holder.deviceId.setText(
        String.format(
            "Device %d (%04X:%04X)", device.getId(), device.getVendorId(), device.getProductId()));
