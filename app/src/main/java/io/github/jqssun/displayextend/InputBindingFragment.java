...
public class InputBindingFragment extends Fragment {
    ...
    bindBtn.setOnClickListener(
        v -> {
          ...
          if (pos != -1 && pos < displayList.size()) {
            State.startNewJob(
                new BindAllExternalInputToDisplay(displayList.get(pos).getDisplayId()));
          }
        });
  }
  ...
  private void _setupDeviceLists() {
    ...
    externalDevicesRecycler.setAdapter(
        new DeviceAdapter(external, this::_showDeviceDetails, R.drawable.ic_external_input_device));
    internalDevicesRecycler.setAdapter(
        new DeviceAdapter(internal, this::_showDeviceDetails, R.drawable.ic_phone));
  }
