...
public class InputDeviceDetailFragment extends Fragment {
  ...
  public View onCreateView(
    ...
    if (device != null) {
      ...
      InfoRow.add(
          ctx, infoTable, getString(R.string.info_device_id), String.valueOf(device.getId()));
      InfoRow.add(
          ctx,
          infoTable,
          getString(R.string.info_product_id),
          String.format("0x%04X", device.getProductId()));
      InfoRow.add(
          ctx,
          infoTable,
          getString(R.string.info_vendor_id),
          String.format("0x%04X", device.getVendorId()));
      InfoRow.add(ctx, infoTable, getString(R.string.info_descriptor), device.getDescriptor());
      InfoRow.add(
          ctx,
          infoTable,
          getString(R.string.info_external),
          PlatformCompat.isExternalInputDevice(device)
  ...
  private void _initDisplaySpinner() {
    ...
    for (Display display : displays) {
      displayNames.add(
          getString(R.string.display_spinner_format, display.getDisplayId(), display.getName()));
    }

    ArrayAdapter<String> adapter =
        new ArrayAdapter<>(requireContext(), android.R.layout.simple_spinner_item, displayNames);
    ...
    bindBtn.setOnClickListener(
        v -> {
          if (!ShizukuUtils.hasShizukuStarted()) {
            Toast.makeText(
                    requireContext(), getString(R.string.shizuku_required), Toast.LENGTH_SHORT)
                .show();
            return;
          }
  ...
  private String _getDeviceSources(InputDevice device) {
    ...
    if (device.supportsSource(InputDevice.SOURCE_KEYBOARD))
      sources.add(getString(R.string.source_keyboard));
    if (device.supportsSource(InputDevice.SOURCE_DPAD))
      sources.add(getString(R.string.source_dpad));
    if (device.supportsSource(InputDevice.SOURCE_GAMEPAD))
      sources.add(getString(R.string.source_gamepad));
    if (device.supportsSource(InputDevice.SOURCE_TOUCHSCREEN))
      sources.add(getString(R.string.source_touchscreen));
    if (device.supportsSource(InputDevice.SOURCE_MOUSE))
      sources.add(getString(R.string.source_mouse));
