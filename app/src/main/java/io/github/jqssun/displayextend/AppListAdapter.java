...
public class AppListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
  ...
  public AppListAdapter(
      List<AppEntry> installedApps,
      List<AppEntry> runningApps,
  ...
  private void _rebuildItems() {
    ...
    if (!filteredRunningApps.isEmpty()) {
      filteredItems.add(
          new ListItem(VIEW_TYPE_HEADER, _getString(R.string.running_tasks_header), null));
    ...
    if (!filteredInstalledApps.isEmpty()) {
      filteredItems.add(
          new ListItem(VIEW_TYPE_HEADER, _getString(R.string.installed_apps_header), null));
  ...
  private void _castApp(View v, ApplicationInfo app) {
    if (!ShizukuUtils.hasPermission() && Pref.getLaunchTime(app.packageName) == 0) {
      ...
      Toast.makeText(
              v.getContext(),
              v.getContext().getString(R.string.first_launch_hint),
              Toast.LENGTH_SHORT)
          .show();
      return;
    }
