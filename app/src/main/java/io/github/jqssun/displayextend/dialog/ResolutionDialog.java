...
public class ResolutionDialog {
  public static void show(Context context, int displayId, int currentWidth, int currentHeight) {
    ...
    TextInputEditText widthInput = dialogView.findViewById(R.id.width_input);
    TextInputEditText heightInput = dialogView.findViewById(R.id.height_input);
            ...
            (dialog, which) -> {
              try {
                ...
                if (newWidth <= 0 || newHeight <= 0) {
                  Toast.makeText(
                          context, context.getString(R.string.invalid_number), Toast.LENGTH_SHORT)
                      .show();
                  return;
                }
